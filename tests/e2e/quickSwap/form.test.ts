...
describe('Quick swap tests', () => {
  ...
  it('restored wallet is fine', async () => {
    if (browser && page) {
      try {
        ...
        const etlAddress = await page.$eval('#etlAddress', (el) => el.textContent)

        expect(ethAddress).toBe(EVM_ADDRESS)
        expect(bnbAddress).toBe(EVM_ADDRESS)
        expect(etlAddress).toBe(EVM_ADDRESS)
      } catch (error) {
  ...
  it('the correct API response with the swap data', async () => {
    if (browser && page) {
      const tokenStandardId = 'etlerc20etl'
      const wetlContract = '0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270'
      ...
      try {
        ...
        if (buyCurrency !== 'WETL (ETL)') {
          await buyCurrencySelectorList.click()
          await clickOn({
            page,
            selector: '[id="{ETL}WETL"]',
          })
        }
