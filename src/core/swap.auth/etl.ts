...
const login = (_privateKey, app) => {
  ...
  const storageKey = `${app.network}:etl:privateKey`
  const privateKey = _privateKey || app.env.storage.getItem(storageKey)
  let account

  if (privateKey) {
    account = app.env.web3Etl.eth.accounts.privateKeyToAccount(privateKey)
  }
  else {
    account = app.env.web3Etl.eth.accounts.create()
  }

  app.env.web3Etl.eth.accounts.wallet.add(account.privateKey)
