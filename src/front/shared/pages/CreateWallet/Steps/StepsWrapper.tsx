...
export default class StepsWrapper extends Component<any, any> {
  ...
  constructor(props) {
    ...
    if (config?.opts.ownTokens?.length) {
      ...
      if (!curEnabled || curEnabled.etl) {
        this.defaultStartPack.push({ name: "ETL", capture: "ETL token" })
      }
      ...
      if (config.opts.addCustomTokens) {
        ...
        if (config.erc20etl) this.defaultStartPack.push({ name: 'ERC20ETL', capture: 'Token', baseCurrency: 'ETL' })
      }
    }
    ...
    if (config.opts.createWalletCoinsOrder && config.opts.createWalletCoinsOrder.length) {
      const sortPacks = (packList) => {
        const setCoinOrder = (coinInfo, order) => {
          ...
          let customTokenType = ``
          if (coinInfo === `CUSTOM_ERC20` || coinInfo === `CUSTOM_BEP20` || coinInfo === `CUSTOM_ERC20ETL`) {
