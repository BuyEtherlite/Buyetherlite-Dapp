...
const routes = (
  <ScrollToTop>
    <Switch>
      ...
      <Route path={`/:ticker(btc|eth|bnb|etl|arbeth|ghost|next)/tx/:tx?`} component={Transaction} />
      <Route path={`/:token(token)/:ticker/tx/:tx?`} component={Transaction} />

      <Route
        path={`/:ticker(btc|eth|bnb|etl|arbeth|ghost|next)/:address/:action(receive|send)?`}
