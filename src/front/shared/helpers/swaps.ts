...
const allowedCoins = [
  ...
  ...(!config.opts.blockchainSwapEnabled || config.opts.blockchainSwapEnabled.etl ? ['ETL'] : []),
...
const isExchangeAllowed = (currencies) =>
  currencies.filter((c) => {
    ...
    const isBep = Object.keys(config.bep20)
      ...
      .includes(`${c.value}`.toLowerCase())
    const isErcEtl = Object.keys(config.erc20etl)
      .map((i) => `{etl}${i.toLowerCase()}`)
      .includes(`${c.value}`.toLowerCase())

    const isAllowedCoin = allowedCoins.map((i) => i.toLowerCase()).includes(c.value.toLowerCase())

    return isAllowedCoin || isErc || isBep || isErcEtl
  })
