  ...
  await metamask.web3connect.onInit(async () => {
    ...
    SwapApp.setup({
      ...
      env: {
        ...
        web3Etl: actions.etl.getWeb3(),
        getWeb3Etl: actions.etl.getWeb3,
      ...
      services: [
        new SwapAuth({
          ...
          etl: localStorage.getItem(privateKeys.privateKeyNames.eth),
      ...
      swaps: [
        ...
        ...((config?.opts?.blockchainSwapEnabled?.etl) ? [
          new EtlSwap({
            address: config.swapContract.etl,
            abi: EVM_CONTRACTS_ABI.NATIVE_COIN_SWAP,
            fetchBalance: (address) => actions.etl.fetchBalance(address),
            estimateGasPrice: () => ethLikeHelper.etl.estimateGasPrice(),
            sendTransaction: ({ to, amount }) => actions.etl.send({ to, amount }),
          })
        ] : []),
        ...
        // Etl
        ...(Object.keys(config.erc20etl)
        .map(key =>
          new EtlTokenSwap({
            name: key,
            tokenAbi: abi,
            address: config.swapContract.erc20etl,
            //@ts-ignore
            decimals: config.erc20etl[key].decimals,
            tokenAddress: config.erc20etl[key].address,
            fetchBalance: (address) => actions.erc20etl.fetchBalance(address, config.erc20etl[key].address, config.erc20etl[key].decimals),
            //@ts-ignore
            estimateGasPrice: ({ speed } = {}) => erc20Like.erc20etl.estimateGasPrice({ speed }),
            abi: EVM_CONTRACTS_ABI.TOKEN_SWAP,
          })
        )),
      ],
      flows: [
        ...
        ...(Object.keys(config.bep20))
          ...
          ...(Object.keys(config.erc20etl))
          .map(key => ETLTOKEN2BTC(key)),
        ...(Object.keys(config.erc20etl))
          .map(key => BTC2ETLTOKEN(key)),
